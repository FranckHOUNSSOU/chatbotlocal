  ...
  return (
    <div className="flex min-h-screen bg-[#f0f4f8]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Topbar — cachée sur mobile quand conversation ouverte */}
        <div className={`bg-white border-b border-[#e5eaf0] px-6 h-14 items-center justify-between sticky z-30 lg:top-0 lg:flex ${selected ? 'hidden' : 'flex top-14'}`}>
        ...
        {/* MOBILE */}
        <div className="lg:hidden flex flex-col" style={{ height: 'calc(100dvh - 56px)', marginTop: selected ? '0' : '0' }}>

          {/* Liste — visible quand pas de conversation sélectionnée */}
          {!selected && (
            <div className="flex-1 overflow-y-auto bg-white mt-14">
              ...
              {phones.map(phone => {
                ...
                return (
                  <div key={phone} onClick={() => setSelected(phone)}
                    className="px-4 py-3 cursor-pointer border-b border-[#f0f4f8] active:bg-[#f0faf4]">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="text-sm font-semibold text-[#1a2942] truncate">{phone.replace('whatsapp:', '')}</span>
                          {admin && (
          ...
          {/* Conversation — visible quand sélectionnée, prend tout l'écran sous la navbar */}
          {selected && (
            <div className="flex flex-col bg-[#f8fafc]" style={{ height: 'calc(100dvh - 56px)', marginTop: '56px' }}>
              {/* Header conversation */}
              <div className="px-4 py-3 bg-white border-b border-[#e5eaf0] flex items-center gap-3 flex-shrink-0">
                <button onClick={() => setSelected(null)}
                  className="w-9 h-9 flex items-center justify-center rounded-full bg-[#f0f4f8] flex-shrink-0">
        ...
        <div className="hidden lg:flex flex-1 overflow-hidden" style={{ height: 'calc(100vh - 56px)' }}>
          <div className="w-80 flex-shrink-0 border-r border-[#e5eaf0] overflow-y-auto bg-white">
            {loading && <div className="p-5 text-sm text-[#8899bb]">Chargement...</div>}
            {!loading && phones.length === 0 && (
              <div className="p-10 text-center text-sm text-[#8899bb]">Aucune conversation</div>
            )}
            {phones.map(phone => {
              ...
              return (
                <div key={phone} onClick={() => setSelected(phone)}
                  className={`px-4 py-3 cursor-pointer border-b border-[#f0f4f8] transition-colors ${selected === phone ? 'bg-[#f0faf4] border-l-2 border-l-[#25D366]' : 'hover:bg-[#f8fafc]'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${admin ? 'bg-[#fff8e1] text-[#b8860b] ring-2 ring-[#ffd700]' : 'bg-[#e6f7ee] text-[#0a7c3e]'}`}>
                      {phone.replace('whatsapp:+', '').slice(-2).toUpperCase()}
                    </div>
